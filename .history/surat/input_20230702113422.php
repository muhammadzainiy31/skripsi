...
<body>
                            ...
                            <div class="card-body">
                                <div class="basic-form">
                                    <form method="POST" action="" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
    <h4><label for="kode">Kode Barang</label></h4>
    <select name="kode" id="kode" class="form-control" required>
        ...
        while ($hasil = mysqli_fetch_array($ambilData)) {
            echo '<option value="' . $hasil['kode'] . '">' . $hasil['kode'] . ' - ' . $hasil['nama_brg'] . '</option>';
        }
        ?>
    </select>
</div>
...
<script>
    // Fungsi untuk melakukan autofill pada nmbrg
    function autofillBrg() {
        var kode_brg = document.getElementById("kode").value;
        <?php
        $ambilData = mysqli_query($conn, "SELECT * FROM tb_barang") or die(mysqli_error($conn));
        while ($hasil = mysqli_fetch_array($ambilData)) {
            echo "if (kode_brg === '" . $hasil['kode'] . "') {";
            echo "document.getElementById('nmbrg').value = '" . $hasil['nama_brg'] . "';";
            echo "}";
        }
        ?>
    }
    ...
    // Panggil fungsi autofill saat combo box berubah
    document.getElementById("kode").addEventListener("change", autofillBrg);
</script>
...
if (isset($_POST['simpan'])) {
    ...
    $input = "INSERT INTO tb_surat (id_surat, nm_cust, no_telpon, alamat_cust, kelurahan, rute, kode, qty, tanggal_kirim) VALUES ('$id_surat$nm_cust', '$no_telpon', '$alamat_cust', '$kelurahan', '$rute', '$kode', '$qty', '$tanggal_kirim')";
